...
### 1. 직접 세그먼트 파싱 (Literal Segment Parsing)
UUID의 표준 형식(`8-4-4-4-12`)을 활용하여 데이터를 직접 읽어옵니다.
송신측(Android `ble-advertiser`)은 다음 배치로 UUID를 생성합니다:

```
{카드[0:8]}-{카드[8:12]}-{카드[12:16]}-0000-{전화뒤4자리}00805F9B
```

- **카드번호 (Card Number):** 앞쪽 세 개 세그먼트(8+4+4 = 총 16자리)를 결합합니다.
  - 예: `12345678-1234-5678-0000-123400805f9b` -> `1234567812345678`
- **전화번호 (Phone Number):** 다섯 번째 세그먼트의 앞 4자리입니다. **뒤 4자리만 송출됩니다.**
  - 예: 위 UUID -> `1234`
  - 레거시 iOS 송신기는 네 번째 세그먼트에 이 4자리를 넣었으며, 이 경우도 함께 지원합니다.

> 네 번째 세그먼트 `0000`은 카드번호와 전화번호 사이의 고정 패딩이며 데이터가 아닙니다.
