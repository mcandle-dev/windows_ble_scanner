...
- **GATT 정보 로깅:** 터미널 콘솔을 통해 발견된 모든 서비스 UUID와 각 서비스에 속한 특성(Characteristic) UUID, 그리고 해당 특성의 속성(Read/Write 등)을 상세히 출력합니다.
...
- **대상 특성 선정:** 연결된 기기의 모든 서비스를 순회하며 `WRITE` 또는 `WRITE-WITHOUT-RESPONSE` 권한이 있는 첫 번째 특성을 자동으로 타겟으로 설정합니다.
- **인코딩 및 전송:** 입력된 텍스트를 `UTF-8` 바이트로 인코딩하여 해당 GATT 특성에 데이터를 전송합니다.
