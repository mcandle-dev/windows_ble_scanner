...
> 🚨 **현행 `main.py`와 불일치**: `decode_uuid_data`는 세그먼트 1–3을 *전화번호*로,
> 세그먼트 4를 *카드번호*로 읽는다. 실제로는 1–3이 카드번호이고 세그먼트 4는 패딩(`0000`)이며
> 전화번호는 세그먼트 5 앞 4자리에 있다. 로그의 `DECODED CARD: 0000`이 그 증상이다.
> constitution §3이 이 잘못된 규칙을 원칙으로 승격해 둔 상태 — 수정 시 함께 고쳐야 한다.
